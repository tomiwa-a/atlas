      ...
      <header className="sticky top-0 bg-secondary bg-opacity-80 backdrop-blur-sm z-10">
        <div className="container mx-auto flex justify-between items-center p-4 text-primary">
          ...
          <nav className="hidden tablet:flex space-x-1 items-center bg-neutral p-2 rounded-full">
            {navLinks.map((link) => (
              <a
                key={link.id}
                href={`#${link.id}`}
                className={`px-3 py-2 tablet:px-5 tablet:py-2 text-lg font-medium transition rounded-full ${
          ...
          <div className="hidden tablet:flex items-center space-x-4">
            <a href="#signup" className="text-lg font-medium hover:opacity-75 transition">Sign Up</a>
            <button className="bg-primary text-secondary px-5 py-2.5 rounded-full font-semibold hover:opacity-90 transition">Sign In</button>
          </div>
